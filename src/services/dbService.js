const STORAGE_KEY = 'netcheck_pcr_reports';

/**
 * Service to handle data persistence (currently using localStorage)
 */
...
export const dbService = {
   ...
   */
  getStats: (providedReports) => {
    const reports = providedReports || dbService.getReports();
    
    // Distribution by label
    const labels = ['Baik', 'Cukup', 'Buruk', 'Sangat Buruk'];
    const chartData = labels.map(label => ({
      name: label,
      value: reports.filter(r => r.label === label).length
    }));

    const avgSpeed = reports.length > 0 
      ? reports.reduce((acc, r) => acc + (parseFloat(r.speed) || 0), 0) / reports.length 
      : 0;
    ...
    return {
      ...
      topLocation: dbService.getTopLocation(reports),
      locationAnalysis: dbService.getLocationAnalysis(reports)
    };
  },
  ...
  getRecommendation: (currentLocation) => {
    const reports = dbService.getReports();
    const analysis = dbService.getLocationAnalysis(reports);
    
    const bestAlternatives = analysis
      .filter(a => a.location !== currentLocation)
      .sort((a, b) => b.baik - a.baik);
    
    return bestAlternatives.length > 0 ? bestAlternatives[0].location : "Area Masjid (Sinyal Stabil)";
  },

  /**
   * Initialize with the complete dataset from Excel
   */
  init: () => {
    const currentReports = dbService.getReports();
    ...
    const newExcelData = normalizedExcelData.filter(r => !existingIds.has(r.id));

    if (newExcelData.length > 0) {
      const mergedReports = [...currentReports, ...newExcelData];
      localStorage.setItem(STORAGE_KEY, JSON.stringify(mergedReports));
    }
  }
};
