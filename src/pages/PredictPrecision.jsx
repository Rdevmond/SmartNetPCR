    ...
    <div className="min-h-screen bg-mesh pt-24 md:pt-32 pb-20 px-4 md:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10 md:mb-16 animate-slide-up">
          <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-purple-50 text-purple-600 text-[10px] font-black uppercase tracking-widest border border-purple-100 mb-4 md:mb-6">
              ...
              <div className="grid grid-cols-3 md:grid-cols-5 gap-3 md:gap-4">
                {SPEEDS.map((speed) => (
                  <div
                    ...
                    className={`cursor-pointer p-4 md:p-6 rounded-2xl border-2 transition-all flex flex-col items-center justify-center text-center group ${formData.speed === speed.value
                      ? 'border-[#003366] bg-[#003366] text-white shadow-xl'
                      : 'border-gray-50 bg-white hover:border-blue-200 hover:shadow-md'
                      }`}
                  >
                    <span className={`text-sm md:text-lg font-black ${formData.speed === speed.value ? 'text-white' : 'text-[#003366]'}`}>
                      {speed.label}
                    </span>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-3 md:grid-cols-6 gap-3 md:gap-4">
                {activityItems.map((item) => (
                  <div
                    ...
                    className={`cursor-pointer p-3 md:p-5 rounded-2xl border-2 transition-all flex flex-col items-center justify-center text-center space-y-2 md:space-y-3 group ${formData.activity === item.key
                      ? 'border-[#003366] bg-[#003366] text-white shadow-xl'
                      : 'border-gray-50 bg-white hover:border-blue-200 hover:shadow-md'
                      }`}
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-5">
                {Object.entries(COMPLAINTS).map(([key, data]) => (
                  <div
                    ...
                    className={`cursor-pointer p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border-2 transition-all flex items-center justify-between group ${formData.complaint === key
                  ...
                  >
                    <div className="flex items-center space-x-3 md:space-x-4">
                      <div className={`w-9 h-9 md:w-10 md:h-10 rounded-xl flex items-center justify-center ${formData.complaint === key ? 'bg-white/10 text-white' : 'bg-gray-100 text-gray-400'}`}>
                        <AlertCircle size={18} />
                      </div>
                      <span className={`text-sm font-bold ${formData.complaint === key ? 'text-white' : 'text-gray-700'}`}>{data.label}</span>
                    </div>
                    {formData.complaint === key && <CheckCircle2 className="w-5 h-5 text-[#FFD700]" />}
                  </div>
                ))}
              </div>
            </div>
...
export default Predictprecision;